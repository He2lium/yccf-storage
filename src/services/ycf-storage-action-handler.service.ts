  ...
  for (let actionHandle of actions) {
    ...
    switch (action) {
      ...
      case YcfStorageActionEnum.downloadImageAndHandle:
        ...
        await handleImagesService(buf, imageOptions);
        break;

      case YcfStorageActionEnum.justHandleImage:
        await handleImagesService(await GetS3Service(sourceKey), imageOptions);
        break;

      case YcfStorageActionEnum.moveImageAndHandle:
        if (destinationKey) await MoveS3Service(sourceKey, destinationKey);
